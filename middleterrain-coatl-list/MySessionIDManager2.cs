...
namespace middleterraincoatllist
{
	...
	public class MySessionIDManager2 : /*IHttpModule,*/ ISessionIDManager
	{
		...
		void ISessionIDManager.SaveSessionID(HttpContext context, string id, out bool redirected, out bool cookieAdded)
		{
			...
			if (pConfig.Cookieless == HttpCookieMode.UseUri)
			{
				...
				var newUri = request.ApplyUrlModifier(request.Url.ToString(), id);
		...
		static string ExtractSessionId (string path)
		{
			...
			var strPrefix = "SessionID=";

			int posPrefix = path.IndexOf(strPrefix);
			if (posPrefix < 0) return null;

			int posPostfix = path.IndexOf(";", posPrefix);
			if (posPostfix < 0) {
				posPostfix = path.Length;
			}


			posPrefix += strPrefix.Length;
